...
local function read_file(path)
  ...
  if not f then
    error("svg-frag: cannot open file: " .. tostring(err))
  end
  local data = f:read("*a")
  f:close()
  return data
...
Shortcodes["svg-frag"] = function(args, kwargs, meta)
  ...
  local reduced = kwargs["reduced_motion"] -- e.g., "show-all"

  local svg = read_file(file)

  if reduced == "show-all" then
    -- Do not add fragment classes; emit as-is
    return pandoc.RawBlock('html', svg)
  end

  local combined_class = class_name
  if effect and effect ~= "" then
    combined_class = combined_class .. " " .. effect
  end
